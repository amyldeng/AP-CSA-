...
public class ElevensBoard extends Board {
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
		...
		for(int i = 0;i<selectedCards.size();i++) {
			pv = cardAt(selectedCards.get(0)).pointValue();
			
			for(int j = 0;j<selectedCards.size()-i;j++) {
				if (pv + cardAt(selectedCards.get(j)).pointValue() == 11) {
				return true;
			}
			}
		}
	...
	private boolean containsJQK(List<Integer> selectedCards) {
		int k = selectedCards.size();
		for (int i = 0; i < selectedCards.size(); i++) {
			if (cardAt(selectedCards.get(i)).rank().equalsIgnoreCase("j")) {
		...
		for (int i = 0; i < selectedCards.size(); i++) {
			if (cardAt(selectedCards.get(i)).rank().equalsIgnoreCase("q")) {
				selectedCards.remove(i);
			}
		}
		if (selectedCards.size() == k) {
		...
		for (int i = 0; i < selectedCards.size(); i++) {
			if (cardAt(selectedCards.get(i)).rank().equalsIgnoreCase("k")) {
				selectedCards.remove(i);
			}
		}
		if (selectedCards.size() == k) {
